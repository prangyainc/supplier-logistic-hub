sap.ui.define([
  ...
  return Controller.extend("inc.lch.FUS.FreightUnits_Supplier.controller.Home", {
    ...
    fnDelete: function (sAction) {
      var that = this;
      // var oItem = sAction.getSource().getParent();
      //       var oTable = oItem.getParent();
      //       var oIndex = oTable.indexOfItem(oItem);
      //       if (oIndex !== -1) {
      //         var model = oTable.getModel("oTableModel");
        ...
        onClose: function (sAction) {
          ...
          if (action === "OK") {
            ...
            for (var i = aSelectedItems.length - 1; i >= 0; i--) { //start with highest index first 
              ...
              var oJSONData = this.getView().getModel("oTableModel").getData();
              oJSONData.correlationData.splice(iIndex, 1); //Use splice to remove your object in the array
              this.getView().getModel("oTableModel").setData(oJSONData); //And set the new data to the model
